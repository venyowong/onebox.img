...
私有云笔记功能的使用场景应该不太可能只局限于家里，所以需要先从配置内网穿透开始，请参考 [使用 DDNSTO 服务将 1Box 服务暴露到外网](advanced.md) 先完成内网穿透的配置
...
1. 同步目标选择 WebDAV
2. URL 按照 `{域名}/cloud/dav/{目录}` 的格式进行填写，域名就是刚刚配置的 DDNSTO 域名，目录就是您刚刚创建的云目录
